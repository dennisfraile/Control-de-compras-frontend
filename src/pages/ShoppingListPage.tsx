...
import { StaggerList, StaggerItem, launchConfetti } from '../components/common/FeedbackAnimations';
...
export default function ShoppingListPage() {
  ...
  // #7 - Set of favorite product IDs for priority sorting
  const favoriteIds = useMemo(() => {
    if (!favorites || !Array.isArray(favorites)) return new Set<string>();
    return new Set(favorites.map((f: any) => f.id || f.productId));
  }, [favorites]);
  ...
  // Build local items - #7 favorites sorted first
  const localItems: LocalItem[] = useMemo(() => {
    ...
    // Sort: favorites first, then by reason urgency
    items.sort((a, b) => {
      const aFav = favoriteIds.has(a.productId) ? 0 : 1;
      const bFav = favoriteIds.has(b.productId) ? 0 : 1;
      return aFav - bFav;
    });
    return items;
  }, [rawItems, checkedSet, qtyOverrides, favoriteIds]);
  ...
  const handleToggle = useCallback((id: string) => {
    setCheckedSet((prev) => {
      ...
      // #18 - Confetti when all items checked
      if (rawItems && next.size === rawItems.length) {
        launchConfetti();
      }
      return next;
    });
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Error al compartir la lista');
    } finally {
      setSharing(false);
    }
  }, []);
